...
<script>
    ...
    function itemsDetailDynamicField(number) {
        ...
        html += '<td><select class="select2 form-control tax" name="items[' + number + '][tax]" id="tax_' + number + '" onchange="calculateCost()" style="width: 100%; height:30px;" data-placeholder="Select Tax Code"><option value="10">GST 10%</option><option value="0">N/A</option></select></td>';
        if (number > 1) {
            html += '<td><button style="border: none; background-color: #fff" name="addItems" id="addItems"><i class="fa fa-plus-circle"></i></button></td>';
            html += '<td><button style="border: none; background-color: #fff" name="removeItems" id="removeItems"><i class="fa fa-minus-circle"></i></button></td></tr>';
            $('#itemsDetailTableBody').append(html);
    ...
    function fetchNotes() {
        $.ajax({
            ...
            success: function(response) {
                ...
                note_id.append($("<option />").val(0).text('Select Notes').prop({
                    selected: true,
                    disabled: true
                }));
