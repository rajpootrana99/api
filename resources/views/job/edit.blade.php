...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <form method="post" action="{{route('quote.update', ['quote' => $invoice->id ])}}" enctype="multipart/form-data">
                    <div class="card-body">
                        @csrf
                        @method('PUT')
                        <div class="row">
...
<script>
    var quotes = <?php echo $invoice->task->quotes ?>;
    ...
    $.each(estimates, function(key, estimate){ 
        ...
        $.each(quotes, function(key, quote) {
            estimate.add(new Option(quote.estimate.sub_header.cost_code + '___' + quote.estimate.item, quote.id));
        });
        estimate.value = quotes[key].id
        let i = key+1
        $('#description_' + i).val(quotes[key].description);
        $('#amount_' + i).val(quotes[key].subtotal);
    });
    ...
    function quoteInsert(){
        ...
        $.each(quoteList, function(key, quote) {
            if(quote.id == quote_id){
                ...
                $('#amount_' + itemsCount).val(quote.subtotal);
            }
        });
    }
